...
- **음식 사진은 위키백과 '참고 이미지'지 그 가게의 음식이 아니다**(`web/src/lib/dishImage.ts`). 썸네일에 '참고' 배지, 상세에 출처와 "이 가게의 실제 음식 사진이 아니에요"를 붙인다. 채택 규칙은 `titleMatches`: 가나 정규화 후 위키 제목과 `name_local` 이 서로를 포함하면 같은 요리로 본다(실측 커버리지 13/42 → 28/42). 글자가 겹치지 않는 오답은 계속 걸러진다 — `グルクン唐揚`(자리돔)에 から揚げ(닭튀김), `フーチャンプルー`에 프로레슬러 페이지가 안 붙는다. **규칙을 손대면 실제 메뉴명으로 적중률·오답을 다시 재라**(위키백과 CORS 는 브라우저에서 열림).
