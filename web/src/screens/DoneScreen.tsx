...
function Done({ scan }: { scan: MenuScanResponse }) {
  ...
  return (
    <div className="flex min-h-full flex-col items-center justify-center gap-5 p-8 text-center">
      ...
      <div>
        <p className="text-[23px] font-extrabold -tracking-[0.4px]">맛있게 드세요!</p>
        <p className="mt-2 text-sm leading-[1.55] text-muted">
          주문이 정리됐어요.
          <br />이 화면을 점원에게 보여줘도 좋아요.
        </p>
      </div>
      ...
      <div className="w-full rounded-2xl border border-line bg-white p-[15px] text-left">
        <div className="flex justify-between text-[13px]">
          ...
          <b>
            ...
            {krwTotal != null && ` · ${formatKrw(krwTotal)}`}
          </b>
        </div>
        {restaurantBudget > 0 && krwTotal != null && (
          <div className="mt-2 flex justify-between text-[13px]">
            <span className="text-muted">이번 식당 예산 잔액</span>
            <b className="text-sage-700">{formatKrw(Math.max(0, restaurantBudget - krwTotal))}</b>
          </div>
        )}
      </div>
